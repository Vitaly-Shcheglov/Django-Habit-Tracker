...
class UniqueValidator:
    ...
    def __repr__(self):
        return "<%s(queryset=%s)>" % (self.__class__.__name__, smart_repr(self.queryset))
...
class ProhibitSurrogateCharactersValidator:
    ...
    def __call__(self, value):
        for surrogate_character in (ch for ch in str(value) if 0xD800 <= ord(ch) <= 0xDFFF):
...
class BaseUniqueForValidator:
    ...
    def enforce_required_fields(self, attrs):
        ...
        """
        missing_items = {
            field_name: self.missing_message for field_name in [self.field, self.date_field] if field_name not in attrs
        }
        if missing_items:
            raise ValidationError(missing_items, code="required")
