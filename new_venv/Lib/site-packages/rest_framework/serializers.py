...
from rest_framework.fields import CreateOnlyDefault, CurrentUserDefault, SkipField, empty  # NOQA # isort:skip
...
def as_serializer_error(exc):
    ...
    if isinstance(detail, Mapping):
        ...
        return {key: value if isinstance(value, (list, Mapping)) else [value] for key, value in detail.items()}
...
class Serializer(BaseSerializer, metaclass=SerializerMetaclass):
    ...
    def get_initial(self):
        ...
        return OrderedDict(
            [(field.field_name, field.get_initial()) for field in self.fields.values() if not field.read_only]
        )
    ...
    * Automatically create a profile instance.
    ...
    assert not any(
        ...
        "nested serializer fields.".format(
            method_name=method_name, module=serializer.__class__.__module__, class_name=serializer.__class__.__name__
        )
    )
    ...
    * Default `.create()` and `.update()` implementations are provided.
    ...
    def build_standard_field(self, field_name, model_field):
        ...
        if postgres_fields and isinstance(model_field, postgres_fields.ArrayField):
            ...
            child_field_class, child_field_kwargs = self.build_standard_field("child", child_model_field)
    ...
    def build_unknown_field(self, field_name, model_class):
        ...
        """
        raise ImproperlyConfigured("Field name `%s` is not valid for model `%s`." % (field_name, model_class.__name__))
    ...
    def get_uniqueness_extra_kwargs(self, field_names, declared_fields, extra_kwargs):
        ...
        model = getattr(self.Meta, "model")
        model_fields = self._get_model_fields(field_names, declared_fields, extra_kwargs)
    ...
    def get_validators(self):
        ...
        return self.get_unique_together_validators() + self.get_unique_for_date_validators()

    def get_unique_together_validators(self):
        """
        Determine a default set of validators for any unique_together constraints.
        """
        model_class_inheritance_tree = [self.Meta.model] + list(self.Meta.model._meta.parents)
        ...
        for parent_class in model_class_inheritance_tree:
            for unique_together in parent_class._meta.unique_together:
                ...
                validator = UniqueTogetherValidator(queryset=parent_class._default_manager, fields=field_names)
        ...
        for field_name, field in info.fields_and_pk.items():
            if field.unique_for_date and field_name in field_names:
                validator = UniqueForDateValidator(
                    queryset=default_manager, field=field_name, date_field=field.unique_for_date
                )
                validators.append(validator)

            if field.unique_for_month and field_name in field_names:
                validator = UniqueForMonthValidator(
                    queryset=default_manager, field=field_name, date_field=field.unique_for_month
                )
                validators.append(validator)

            if field.unique_for_year and field_name in field_names:
                validator = UniqueForYearValidator(
                    queryset=default_manager, field=field_name, date_field=field.unique_for_year
                )
                validators.append(validator)
