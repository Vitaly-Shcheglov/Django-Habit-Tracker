...
def _get_page_links(page_numbers, current, url_func):
    ...
    for page_number in page_numbers:
        if page_number is None:
            ...
            page_link = PageLink(
                url=url_func(page_number), number=page_number, is_active=(page_number == current), is_break=False
            )
...
class LimitOffsetPagination(BasePagination):
    ...
    def get_limit(self, request):
        if self.limit_query_param:
            try:
                return _positive_int(request.query_params[self.limit_query_param], strict=True, cutoff=self.max_limit)
            except (KeyError, ValueError):
                pass
    ...
    def get_html_context(self):
        ...
        if self.limit:
            ...
            final = _divide_with_ceil(self.count - self.offset, self.limit) + _divide_with_ceil(
    ...
    def get_schema_fields(self, view):
        ...
        return [
            coreapi.Field(
                ...
                required=False,
                location="query",
                schema=coreschema.Integer(title="Limit", description=force_str(self.limit_query_description)),
            ),
            coreapi.Field(
                name=self.offset_query_param,
                required=False,
                location="query",
                schema=coreschema.Integer(title="Offset", description=force_str(self.offset_query_description)),
            ),
        ]
...
class CursorPagination(BasePagination):
    ...
    def get_ordering(self, request, queryset, view):
        ...
        if ordering_filters:
            ...
            ), "Using cursor pagination, but filter class {filter_cls} " "returned a `None` ordering.".format(
        ...
        ), "Invalid ordering. Expected string or tuple, but got {type}".format(type=type(ordering).__name__)
