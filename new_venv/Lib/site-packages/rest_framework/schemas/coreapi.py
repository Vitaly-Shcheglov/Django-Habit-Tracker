...
def is_custom_action(action):
    return action not in {"retrieve", "list", "create", "update", "partial_update", "destroy"}
...
def insert_into(target, keys, value):
    ...
    try:
        ...
        msg = INSERT_INTO_COLLISION_FMT.format(value_url=value.url, target_url=target.url, keys=keys)
...
class SchemaGenerator(BaseSchemaGenerator):
    ...
    def get_schema(self, request=None, public=False):
        ...
        return coreapi.Document(title=self.title, description=self.description, url=url, content=links)
    ...
    def get_keys(self, subpath, method, view):
        ...
        named_path_components = [component for component in subpath.strip("/").split("/") if "{" not in component]
...
def field_to_schema(field):
    ...
    if isinstance(field, (serializers.ListSerializer, serializers.ListField)):
        ...
        return coreschema.Array(items=child_schema, title=title, description=description)
    elif isinstance(field, serializers.DictField):
        return coreschema.Object(title=title, description=description)
    elif isinstance(field, serializers.Serializer):
        return coreschema.Object(
            properties=OrderedDict([(key, field_to_schema(value)) for key, value in field.fields.items()]),
            title=title,
        ...
        return coreschema.Array(items=related_field_schema, title=title, description=description)
        ...
        return coreschema.Array(items=coreschema.Enum(enum=list(field.choices)), title=title, description=description)
    elif isinstance(field, serializers.ChoiceField):
        return coreschema.Enum(enum=list(field.choices), title=title, description=description)
        ...
        return coreschema.String(title=title, description=description, format="date-time")
    elif isinstance(field, serializers.JSONField):
        return coreschema.Object(title=title, description=description)

    if field.style.get("base_template") == "textarea.html":
        return coreschema.String(title=title, description=description, format="textarea")
...
class AutoSchema(ViewInspector):
    ...
    def get_serializer_fields(self, path, method):
        ...
        if isinstance(serializer, serializers.ListSerializer):
            return [coreapi.Field(name="data", location="body", required=True, schema=coreschema.Array())]
        ...
        for field in serializer.fields.values():
            ...
            field = coreapi.Field(
                name=field.field_name, location="form", required=required, schema=field_to_schema(field)
            )
            fields.append(field)
