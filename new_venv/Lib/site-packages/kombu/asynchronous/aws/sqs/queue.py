...
class AsyncQueue:
    ...
    count_slow = dump = save_to_file = save_to_filename = save = save_to_s3 = load_from_s3 = load_from_file = (
        load_from_filename
    ) = load = clear = _NA
    ...
    def get_messages(
        self, num_messages=1, visibility_timeout=None, attributes=None, wait_time_seconds=None, callback=None
    ):
    ...
    def count(self, page_size=10, vtimeout=10, callback=None, _attr="ApproximateNumberOfMessages"):
        return self.get_attributes(
            _attr,
            callback=transform(
                self._coerce_field_value,
