...
class Urllib3Client(BaseClient):
    ...
    def get_pool(self, request: Request):
        ...
        if request.proxy_host:
            ...
            if request.proxy_username:
                proxy_headers = urllib3.make_headers(
                    proxy_basic_auth=(f"{request.proxy_username}" f":{request.proxy_password}")
                )
            else:
                proxy_headers = None
        ...
        # create new pool
        if _proxy_url:
            _pool = urllib3.ProxyManager(proxy_url=_proxy_url, num_pools=self.max_clients, proxy_headers=proxy_headers)
        else:
    ...
    def _process_request(self, request: Request):
        ...
        if request.auth_username is not None:
            headers.update(
                urllib3.util.make_headers(basic_auth=(f"{request.auth_username}" f":{request.auth_password or ''}"))
            )
        ...
        try:
            ...
            response_obj = self.Response(
                request=request, code=599, headers={}, buffer=None, effective_url=None, error=HttpError(599, str(e))
            )
