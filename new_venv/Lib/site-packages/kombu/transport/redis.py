...
def get_redis_error_classes():
    ...
    return error_classes_t(
        ...
        (virtual.Transport.channel_errors + (DataError, exceptions.InvalidResponse, exceptions.ResponseError)),
    )
...
class QoS(virtual.QoS):
    ...
    def _remove_from_indices(self, delivery_tag, pipe=None):
        with self.pipe_or_acquire(pipe) as pipe:
            return pipe.zrem(self.unacked_index_key, delivery_tag).hdel(self.unacked_key, delivery_tag)
            ...
            try:
                with Mutex(client, self.unacked_mutex_key, self.unacked_mutex_expire):
                    visible = client.zrevrangebyscore(
                        self.unacked_index_key, ceil, 0, start=num and start, num=num, withscores=True
                    )
...
class Channel(virtual.Channel):
            ...
            if bytes_to_str(payload["type"]).endswith("message"):
                ...
                if payload["data"]:
                    ...
                    try:
                        ...
                        warning("Cannot process event on channel %r: %s", channel, repr(payload)[:4096], exc_info=1)
                        raise Empty()
    ...
    def _queue_bind(self, exchange, routing_key, pattern, queue):
        ...
        with self.conn_or_acquire() as client:
            client.sadd(
                self.keyprefix_queue % (exchange,), self.sep.join([routing_key or "", pattern or "", queue or ""])
            )
    ...
    def _filter_tcp_connparams(self, socket_keepalive=None, socket_keepalive_options=None, **params):
        return params
    ...
    def _get_client(self):
        if redis.VERSION < (3, 2, 0):
            raise VersionMismatch(
                "Redis transport requires redis-py versions 3.2.0 or later. " "You have {0.__version__}".format(redis)
            )
...
if sentinel:
    ...
    class SentinelManagedSSLConnection(sentinel.SentinelManagedConnection, redis.SSLConnection):
