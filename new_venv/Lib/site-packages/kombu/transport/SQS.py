...
class Channel(virtual.Channel):
        ...
        for queue_name, q in self.predefined_queues.items():
            ...
            if fifo_url and not fifo_name:
                raise InvalidQueueException(
                    "Queue with url '{}' must have a name " "ending with .fifo".format(q["url"])
                )
        ...
        try:
            ...
            if self.predefined_queues:
                raise UndefinedQueueException(
                    ("Queue with name '{}' must be " "defined in 'predefined_queues'.").format(sqs_qname)
                )
    ...
    def _get_bulk(self, queue, max_if_unlimited=SQS_MAX_MESSAGES, callback=None):
        ...
        if max_count:
            ...
            resp = self.sqs(queue=queue).receive_message(
                QueueUrl=q_url, MaxNumberOfMessages=max_count, WaitTimeSeconds=self.wait_time_seconds
            )
    ...
    def _get(self, queue):
        ...
        q_url = self._new_queue(queue)
        resp = self.sqs(queue=queue).receive_message(
            QueueUrl=q_url, MaxNumberOfMessages=1, WaitTimeSeconds=self.wait_time_seconds
        )
        if resp.get("Messages"):
    ...
    def _new_predefined_queue_client_with_sts_session(self, queue, region):
        sts_creds = self.generate_sts_session_token(
            self.transport_options.get("sts_role_arn"), self.transport_options.get("sts_token_timeout", 900)
        )
    ...
    def asynsqs(self, queue=None):
        if queue is not None and self.predefined_queues:
            ...
            if queue not in self.predefined_queues:
                raise UndefinedQueueException(
                    ("Queue with name '{}' must be defined in " "'predefined_queues'.").format(queue)
                )
            q = self.predefined_queues[queue]
