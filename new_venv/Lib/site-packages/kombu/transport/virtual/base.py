...
class Channel(AbstractChannel, base.StdChannel):
    ...
    def __init__(self, connection, **kwargs):
        ...
        self.exchange_types = {typ: cls(self) for typ, cls in self.exchange_types.items()}
        ...
        try:
            ...
            if not self.typeof(exchange).equivalent(prev, exchange, type, durable, auto_delete, arguments):
                raise NotEquivalentError(
                    NOT_EQUIVALENT_FMT.format(exchange, self.connection.client.virtual_host or "/")
                )
        except KeyError:
    ...
    def queue_declare(self, queue=None, passive=False, **kwargs):
        ...
        if passive and not self._has_queue(queue, **kwargs):
            raise ChannelError(
                "NOT_FOUND - no queue {!r} in vhost {!r}".format(queue, self.connection.client.virtual_host or "/"),
                (50, 10),
    ...
    def queue_bind(self, queue, exchange=None, routing_key="", arguments=None, **kwargs):
    ...
    def basic_publish(self, message, exchange, routing_key, **kwargs):
        ...
        if exchange:
            return self.typeof(exchange).deliver(message, exchange, routing_key, **kwargs)
    ...
    def _lookup(self, exchange, routing_key, default=None):
        ...
        if not R and default is not None:
            warnings.warn(
                UndeliverableWarning(UNDELIVERABLE_FMT.format(exchange=exchange, routing_key=routing_key)),
            )
...
class Transport(base.Transport):
    ...
    def _deliver(self, message, queue):
        if not queue:
            raise KeyError("Received message without destination queue: {}".format(message))
        try:
    ...
    def on_message_ready(self, channel, message, queue):
        if not queue or queue not in self._callbacks:
            raise KeyError("Message for queue {!r} without consumers: {}".format(queue, message))
