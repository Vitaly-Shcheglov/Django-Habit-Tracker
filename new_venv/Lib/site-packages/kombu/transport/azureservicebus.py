...
class Channel(virtual.Channel):
    ...
    def _try_parse_connection_string(self) -> None:
        ...
        if (DefaultAzureCredential is not None and isinstance(self._credential, DefaultAzureCredential)) or (
            ManagedIdentityCredential is not None and isinstance(self._credential, ManagedIdentityCredential)
        ):
            return None
    ...
    def _add_queue_to_cache(
        self, name: str, receiver: ServiceBusReceiver | None = None, sender: ServiceBusSender | None = None
    ) -> SendReceive:
    ...
    def _get_asb_receiver(
        ...
        if queue_obj is None or queue_obj.receiver is None:
            receiver = self.queue_service.get_queue_receiver(
                queue_name=queue, receive_mode=recv_mode, keep_alive=self.uamqp_keep_alive_interval
            )
    ...
    def _purge(self, queue) -> int:
        ...
        while True:
            messages = queue_obj.receiver.receive_messages(max_message_count=max_purge_count, max_wait_time=0.2)
            n += len(messages)
    ...
    def peek_lock_seconds(self) -> int:
        return min(
            self.transport_options.get("peek_lock_seconds", self.default_peek_lock_seconds), 300
...
class Transport(virtual.Transport):
    ...
    def parse_uri(uri: str) -> tuple[str, str | DefaultAzureCredential | ManagedIdentityCredential]:
        ...
        if "DefaultAzureCredential".lower() == credential.lower():
            if DefaultAzureCredential is None:
                raise ImportError(
                    "Azure Service Bus transport with a "
    ...
    def as_uri(cls, uri: str, include_password=False, mask="**") -> str:
        ...
        if isinstance(credential, str) and ":" in credential:
            policy, sas_key = credential.split(":", 1)
            return "azureservicebus://{}:{}@{}".format(policy, sas_key if include_password else mask, namespace)

        return "azureservicebus://{}@{}".format(credential.__class__.__name__, namespace)
