...
class Channel(virtual.Channel):
    ...
    def _new_queue(self, queue, **_):
        ...
        with self._queue_lock(queue):
            try:
                return self.client.write(key=self._key_prefix(queue), dir=True, value=None)
    ...
    def _get(self, queue, timeout=None):
        ...
        with self._queue_lock(queue):
            ...
            try:
                result = self.client.read(key=key, recursive=True, index=self.index, timeout=self.timeout)
...
class Transport(virtual.Transport):
    ...
    implements = virtual.Transport.implements.extend(exchange_type=frozenset(["direct"]))
