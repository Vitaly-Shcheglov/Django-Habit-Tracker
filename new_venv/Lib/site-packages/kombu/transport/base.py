...
def _LeftBlank(obj, method):
    return NotImplementedError(
        "Transport {0.__module__}.{0.__name__} does not implement {1}".format(obj.__class__, method)
    )
...
class Transport:
    ...
    def _make_reader(
        self, connection, timeout=socket.timeout, error=socket.error, _unavail=(errno.EAGAIN, errno.EINTR)
    ):
