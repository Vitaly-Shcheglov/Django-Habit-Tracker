...
class QoS:
    ...
    def can_consume_max_estimate(self):
        ...
        """
        return 1 if not self.prefetch_count else (self.prefetch_count - len(self._not_yet_acked))
...
class Channel(base.StdChannel):
    ...
    def _put(self, routing_key, message, exchange=None, durable=True, **kwargs):
        ...
        if not exchange:
            ...
            address = f"{exchange}/{routing_key}; " "{{assert: always, node: {{type: topic}}}}"
            msg_subject = str(routing_key)
        sender = self.transport.session.sender(address)
        qpid_message = qpid.messaging.Message(content=message, durable=durable, subject=msg_subject)
        try:
    ...
    def queue_declare(
        self, queue, passive=False, durable=False, exclusive=False, auto_delete=True, nowait=False, arguments=None
    ):
    ...
    def prepare_message(
        self, body, priority=None, content_type=None, content_encoding=None, headers=None, properties=None
    ):
...
class Connection:
    ...
    def __init__(self, **connection_options):
        ...
        try:
            msg = _("Attempting to connect to qpid with " "SASL mechanism %s") % sasl_mech
            logger.debug(msg)
            ...
            contains_mech_fail_text = "sasl negotiation failed: no mechanism agreed" in conn_exc.text
            ...
            if (
                ...
                msg = _("Unable to connect to qpid with SASL " "mechanism %s") % sasl_mech
                logger.error(msg)
    ...
    def establish_connection(self):
        ...
        if conninfo.login_method is None:
            if conninfo.userid is not None and conninfo.password is not None:
                ...
                raise Exception(
                    "Password configured but no username. SASL PLAIN " "requires a username when using a password."
                )
            elif conninfo.userid is not None and conninfo.password is None:
                raise Exception(
                    "Username configured but no password. SASL PLAIN " "requires a password when using a username."
                )
            else:
                sasl_mech = "ANONYMOUS"
        else:
        ...
        self.session.set_message_received_notify_handler(self._qpid_message_ready_handler)
        conn.get_qpid_connection().set_async_exception_notify_handler(self._qpid_async_exception_notify_handler)
