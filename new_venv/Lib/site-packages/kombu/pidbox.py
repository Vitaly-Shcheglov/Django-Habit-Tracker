...
class Mailbox:
    ...
    def _broadcast(
        ...
        if destination is not None and not isinstance(destination, (list, tuple)):
            raise ValueError("destination must be a list/tuple not {}".format(type(destination)))
        if (
            pattern is not None
            ...
            raise ValueError("pattern and matcher must be " "strings not {}, {}".format(type(pattern), type(matcher)))
