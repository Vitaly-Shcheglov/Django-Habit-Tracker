...
class Connection:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.failover_strategy = self.failover_strategies.get(self._failover_strategy) or self._failover_strategy
        if self.alt:
    ...
    def switch(self, conn_str):
        ...
        self._closed = False
        conn_params = parse_url(conn_str) if "://" in conn_str else {"hostname": conn_str}
    ...
    def _reraise_as_library_errors(
        self, ConnectionError=exceptions.OperationalError, ChannelError=exceptions.OperationalError
    ):
        try:
            yield
    ...
    def _info(self, resolve=True):
        ...
        if not self.hostname and D.get("hostname"):
            logger.warning("No hostname was supplied. " f"Reverting to default '{D.get('hostname')}'")
    ...
    def SimpleQueue(
        self, name, no_ack=None, queue_opts=None, queue_args=None, exchange_opts=None, channel=None, **kwargs
    ):
        ...
        return SimpleQueue(channel or self, name, no_ack, queue_opts, queue_args, exchange_opts, **kwargs)
    ...
    def connected(self):
        ...
        return not self._closed and self._connection is not None and self.transport.verify_connection(self._connection)
