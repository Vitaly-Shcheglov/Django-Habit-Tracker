...
class SimpleBase:
    ...
    def put(self, message, serializer=None, headers=None, compression=None, routing_key=None, **kwargs):
        self.producer.publish(
            message, serializer=serializer, routing_key=routing_key, headers=headers, compression=compression, **kwargs
        )
...
class SimpleQueue(SimpleBase):
    ...
    def __init__(
        ...
        if not isinstance(queue, entity.Queue):
            ...
            queue = entity.Queue(name, exchange, name, queue_arguments=queue_args, **queue_opts)
            routing_key = name
        else:
        ...
        producer = messaging.Producer(
            channel, exchange, serializer=serializer, routing_key=routing_key, compression=compression
        )
...
class SimpleBuffer(SimpleQueue):
    ...
    exchange_opts = {"durable": False, "delivery_mode": "transient", "auto_delete": True}
