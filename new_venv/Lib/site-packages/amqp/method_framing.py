    ...
    def on_frame(frame):
        ...
        if frame_type not in (expected_types[channel], 8):
            raise UnexpectedFrame(
                "Received frame {} while expecting type: {}".format(frame_type, expected_types[channel]),
            )
        elif frame_type == 1:
...
def frame_writer(
    ...
    def write_frame(type_, channel, method_sig, args, content):
        ...
        if bigbody:
            ...
            frame = b"".join([pack(">HH", *method_sig), args]) if type_ == 1 else b""  # encode method frame
            framelen = len(frame)
            ...
            if body:
                ...
                for i in range(0, bodylen, chunk_size):
                    ...
                    framelen = len(frame)
                    write(pack(">BHI%dsB" % framelen, 3, channel, framelen, frame, 0xCE))
            ...
            framelen = len(frame)
            pack_into(">BHI%dsB" % framelen, buf, offset, type_, channel, framelen, frame, 0xCE)
            offset += 8 + framelen
            if body is not None:
                frame = b"".join(
                    [
                ...
                pack_into(">BHI%dsB" % framelen, buf, offset, 2, channel, framelen, frame, 0xCE)
                offset += 8 + framelen

                bodylen = len(body)
                if bodylen > 0:
                    framelen = bodylen
                    pack_into(">BHI%dsB" % framelen, buf, offset, 3, channel, framelen, body, 0xCE)
                    offset += 8 + framelen
