...
def group_tzcasts(tlist):
    ...
    def valid_next(token):
        return token is not None and (
            token.is_whitespace or token.match(T.Keyword, "AS") or token.match(*sql.TypedLiteral.M_CLOSE)
        )
...
def group_operator(tlist):
    ttypes = T_NUMERICAL + T_STRING + T_NAME
    sqlcls = (sql.SquareBrackets, sql.Parenthesis, sql.Function, sql.Identifier, sql.Operation, sql.TypedLiteral)
...
def group_comments(tlist):
    ...
    while token:
        eidx, end = tlist.token_not_matching(lambda tk: imt(tk, t=T.Comment) or tk.is_newline, idx=tidx)
        if end is not None:
