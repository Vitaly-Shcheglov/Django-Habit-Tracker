...
class Operator(enum.Enum):
    ...
    path_style_parameter = ";"  # 3.2.7. Path-Style Parameter Expansion: {;var}
...
class URIVariable:
    ...
    def parse(self) -> None:
        ...
        for var in var_list:
            ...
            self.variables.append((name, {"explode": explode, "prefix": prefix}))
    ...
    def _query_expansion(
        ...
        if list_test(value) and not tuples:
            ...
            if explode:
                return self.operator.expansion_separator().join(f"{name}={_quote(v)}" for v in value)
            else:
        ...
        if dict_test(value) or tuples:
            ...
            if explode:
                return self.operator.expansion_separator().join(f"{quote(k, safe)}={_quote(v)}" for k, v in items)
            else:
                value = ",".join(f"{quote(k, safe)},{_quote(v)}" for k, v in items)
                return f"{name}={value}"
    ...
    def _label_path_expansion(
        ...
        if value is None or (not isinstance(value, (str, int, float, complex)) and len(value) == 0):
            return None
        ...
        if list_test(value) and not tuples:
            ...
            value = t.cast(t.Sequence[ScalarVariableValue], value)
            fragments = [self.operator.quote(v) for v in value if v is not None]
        ...
        if dict_test(value) or tuples:
            ...
            expanded = join_str.join(
                format_str % (quote(k, safe), self.operator.quote(v)) for k, v in items if v is not None
            )
    ...
    def _semi_path_expansion(
        ...
        if list_test(value) and not tuples:
            value = t.cast(t.Sequence[ScalarVariableValue], value)
            if explode:
                expanded = join_str.join(f"{name}={quote(v, safe)}" for v in value if v is not None)
                return expanded if expanded else None
            else:
        ...
        if dict_test(value) or tuples:
            ...
            if explode:
                return join_str.join(f"{quote(k, safe)}={self.operator.quote(v)}" for k, v in items if v is not None)
            else:
                expanded = ",".join(f"{quote(k, safe)},{self.operator.quote(v)}" for k, v in items if v is not None)
    ...
    def _string_expansion(
        ...
        if dict_test(value) or tuples:
            ...
            return ",".join(format_str % (self.operator.quote(k), self.operator.quote(v)) for k, v in items)

        value = t.cast(t.Text, value)
        value = value[:prefix] if prefix else value
        return self.operator.quote(value)

    def expand(self, var_dict: t.Optional[VariableValueDict] = None) -> t.Mapping[str, str]:
        ...
        for name, opts in self.variables:
            ...
            expanded = None
            if self.operator == Operator.path_segment or self.operator == Operator.label_with_dot_prefix:
                expansion = self._label_path_expansion
            elif self.operator == Operator.form_style_query or self.operator == Operator.form_style_query_continuation:
        ...
        value = ""
        if return_values:
            value = self.operator.variable_prefix() + self.operator.expansion_separator().join(return_values)
