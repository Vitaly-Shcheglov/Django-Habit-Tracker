...
class Worker:
                ...
                if req:
                    ...
                    if max_memory_per_child > 0:
                        ...
                        if used_kb > 0 and used_kb > max_memory_per_child:
                            warning(MAXMEM_USED_FMT.format(used_kb, max_memory_per_child))
                            return EX_RECYCLE
    ...
    def _make_child_methods(self, loads=pickle_loads):
        ...
        self.wait_for_syn = self._make_protected_receive(self.synq) if self.synq else None
...
class ResultHandler(PoolThread):
    ...
    def _make_methods(self):
        ...
        state_handlers = self.state_handlers = {ACK: on_ack, READY: on_ready, DEATH: on_death}
...
class Pool:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.enable_timeouts = bool(enable_timeouts or self.timeout is not None or self.soft_timeout is not None)

        if soft_timeout and SIG_SOFT_TIMEOUT is None:
            warnings.warn(
                UserWarning(
                    "Soft timeouts are not supported: " "on this platform: It does not have the SIGUSR1 signal.",
                )
            )
            soft_timeout = None
    ...
    def _process_by_pid(self, pid):
        return next(((proc, i) for i, proc in enumerate(self._pool) if proc.pid == pid), (None, None))
    ...
    def _join_exited_workers(self, shutdown=False):
        ...
        # WorkerLostError.
        for job in [job for job in list(self._cache.values()) if not job.ready() and job._worker_lost]:
        ...
        if cleaned:
            ...
            for job in list(self._cache.values()):
                acked_by_gone = next((pid for pid in job.worker_pids() if pid in cleaned or pid not in all_pids), None)
    ...
    def mark_as_worker_lost(self, job, exitcode):
        try:
            raise WorkerLostError(
                "Worker exited prematurely: {0} Job: {1}.".format(human_status(exitcode), job._job),
            )
    ...
    def imap(self, func, iterable, chunksize=1, lost_worker_timeout=None):
        ...
        if chunksize == 1:
            ...
            self._taskqueue.put(
                (
                    ((TASK, (result._job, i, func, (x,), {})) for i, x in enumerate(iterable)),
                    result._set_length,
                )
            )
            return result
            ...
            self._taskqueue.put(
                (
                    ((TASK, (result._job, i, mapstar, (x,), {})) for i, x in enumerate(task_batches)),
                    result._set_length,
                )
            )
    ...
    def apply_async(
        ...
        if self._state == RUN:
            ...
            if timeout or soft_timeout:
                ...
                self._start_timeout_handler()
            if self.threads:
                self._taskqueue.put(([(TASK, (result._job, None, func, args, kwds))], None))
            else:
    ...
    def _map_async(self, func, iterable, mapper, chunksize=None, callback=None, error_callback=None):
        ...
        self._taskqueue.put((((TASK, (result._job, i, mapper, (x,), {})) for i, x in enumerate(task_batches)), None))
        return result
