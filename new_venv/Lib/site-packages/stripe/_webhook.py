...
class Webhook(object):
    ...
    @staticmethod
    def construct_event(payload, sig_header, secret, tolerance=DEFAULT_TOLERANCE, api_key=None):
        ...
        event = Event._construct_from(
            values=data,
            requestor=_APIRequestor._global_with_options(api_key=api_key or stripe.api_key),
            api_mode="V1",
        )
...
class WebhookSignature(object):
    ...
    def verify_header(cls, payload, header, secret, tolerance=None):
        try:
            timestamp, signatures = cls._get_timestamp_and_signatures(header, cls.EXPECTED_SCHEME)
        except Exception:
