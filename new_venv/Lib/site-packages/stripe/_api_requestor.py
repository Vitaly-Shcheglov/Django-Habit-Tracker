...
from typing import (Any, AsyncIterable, ClassVar, Dict, List, Mapping,
...
from stripe._requestor_options import RequestorOptions, _GlobalRequestorOptions
from stripe._stripe_response import (StripeResponse, StripeStreamResponse,
                                     StripeStreamResponseAsync)
from stripe._util import (_convert_to_stripe_object, dashboard_link,
                          get_api_mode, log_debug, log_info)
...
class _APIRequestor(object):
    ...
    def handle_error_response(self, rbody, rcode, resp, rheaders, api_mode) -> NoReturn:
        ...
        if err is None:
            err = (
                ...
                if api_mode == "V2"
                else self.specific_v1_api_error(rbody, rcode, resp, rheaders, error_data)
            )
    ...
    def specific_v1_api_error(self, rbody, rcode, resp, rheaders, error_data):
        ...
        if rcode == 429 or (rcode == 400 and error_data.get("code") == "rate_limit"):
            ...
            if error_data.get("type") == "idempotency_error":
                return error.IdempotencyError(error_data.get("message"), rbody, rcode, resp, rheaders)
            else:
            ...
            return error.APIError(error_data.get("message"), rbody, rcode, resp, rheaders)
    ...
    def _args_for_request_with_retries(
        ...
        *,
        ...
        if options and "stripe_version" in options and (options["stripe_version"] is not None):
        ...
        if method == "get" or method == "delete":
            ...
            if options is not None and options.get("content_type") == "multipart/form-data":
                generator = MultipartDataGenerator()
                generator.add_params(params or {})
                post_data = generator.get_post_data()
                headers["Content-Type"] = "multipart/form-data; boundary=%s" % (generator.boundary,)
            else:
    ...
    def request_raw(
        ...
        *,
        ...
        log_debug("Post details", post_data=encoded_params, api_version=api_version)
        ...
        if self._should_handle_code_as_error(rcode):
            if hasattr(stream, "getvalue"):
                ...
                raise NotImplementedError(
                    ...
                    "can be consumed when streaming a response." % self._get_http_client().name
                )
            ...
            raise RuntimeError("_interpret_response should have raised an error")
        else:
