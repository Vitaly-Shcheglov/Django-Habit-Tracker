...
from typing import (Any, AsyncIterator, Generic, Iterator, List, Mapping,
                    TypeVar, cast)
...
class ListObject(StripeObject, Generic[T]):
    ...
    async def previous_page_async(self, **params: Unpack[RequestOptions]) -> Self:
        if not self.has_more:
            request_options, _ = extract_options_from_dict(params)
            return self._empty_list(
                **request_options,
        ...
        result = await self.list_async(**self._get_filters_for_previous_page(params))
        return result
