...
from typing import (Any, AsyncIterable, ClassVar, Dict, Iterable, List,
                    Mapping, MutableMapping, Optional, Tuple, Union, cast,
                    overload)
...
from typing_extensions import Awaitable, Literal, Never, NoReturn, TypedDict
...
class HTTPClient(object):
    ...
    def _should_retry(
        ...
        max_network_retries = max_network_retries if max_network_retries is not None else 0
    ...
    def _add_telemetry_header(self, headers: Mapping[str, str]) -> Mapping[str, str]:
    ...
    def _record_request_metrics(self, response, request_start, usage):
        ...
        if "Request-Id" in rheaders and stripe.enable_telemetry:
            ...
            self._thread_local.last_request_metrics = RequestMetrics(request_id, request_duration_ms, usage=usage)
    ...
    def _request_with_retries_internal(
        ...
        *,
        ...
        while True:
            ...
            if self._should_retry(response, connection_error, num_retries, max_network_retries):
                if connection_error:
                    _util.log_info("Encountered a retryable error %s" % connection_error.user_message)
                num_retries += 1
                sleep_time = self._sleep_time_seconds(num_retries, response)
                _util.log_info(
                    (
                        "Initiating retry %i for request %s %s after "
                        "sleeping %.2f seconds." % (num_retries, method, url, sleep_time)
                    )
                )
                time.sleep(sleep_time)
            else:
                if response is not None:
                    self._record_request_metrics(response, request_start, usage=_usage)
    ...
    def request_stream(
        ...
        *,
        ...
        raise NotImplementedError("HTTPClient subclasses must implement `request_stream`")
    ...
    async def _request_with_retries_internal_async(
        ...
        *,
        ...
        while True:
            ...
            try:
                if is_streaming:
                    response = await self.request_stream_async(method, url, headers, post_data)
                else:
                    response = await self.request_async(method, url, headers, post_data)
                connection_error = None
            except APIConnectionError as e:
                connection_error = e
                response = None
    ...
    async def request_async(
        ...
        if self._async_fallback_client is not None:
            return await self._async_fallback_client.request_async(method, url, headers, post_data)
        raise NotImplementedError("HTTPClient subclasses must implement `request_async`")

    async def request_stream_async(
        self, method: str, url: str, headers: Mapping[str, str], post_data=None
    ) -> Tuple[AsyncIterable[bytes], int, Mapping[str, str]]:
        if self._async_fallback_client is not None:
            return await self._async_fallback_client.request_stream_async(method, url, headers, post_data)
        raise NotImplementedError("HTTPClient subclasses must implement `request_stream_async`")
    ...
    def sleep_async(self, secs: float) -> Awaitable[None]:
        ...
        raise NotImplementedError("HTTPClient subclasses must implement `sleep`")
...
class PycurlClient(HTTPClient):
    ...
    def request(self, method, url, headers: Mapping[str, str], post_data=None) -> Tuple[str, int, Mapping[str, str]]:
        return self._request_internal(method, url, headers, post_data, is_streaming=False)
...
class Urllib2Client(HTTPClient):
    ...
    def __init__(
        ...
        if self._proxy:
            ...
            proxy_handler = urllibrequest.ProxyHandler(cast(Dict[str, str], self._proxy))
    ...
    def _request_internal(
        ...
        try:
            ...
            response = self._opener.open(req) if self._opener else urllibrequest.urlopen(req)
...
class HTTPXClient(HTTPClient):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if httpx is None:
            raise ImportError("Unexpected: tried to initialize HTTPXClient but the httpx module is not present.")

        if anyio is None:
            raise ImportError("Unexpected: tried to initialize HTTPXClient but the anyio module is not present.")
        ...
        kwargs = {}
        if self._verify_ssl_certs:
            kwargs["verify"] = ssl.create_default_context(cafile=stripe.ca_bundle_path)
        else:
            kwargs["verify"] = False
    ...
    def _get_request_args_kwargs(self, method: str, url: str, headers: Mapping[str, str], post_data):
        kwargs = {}
...
class AIOHTTPClient(HTTPClient):
    ...
    def __init__(self, timeout: Optional[Union[float, "AIOHTTPTimeout"]] = 80, **kwargs):
        super(AIOHTTPClient, self).__init__(**kwargs)

        if aiohttp is None:
            raise ImportError("Unexpected: tried to initialize AIOHTTPClient but the aiohttp module is not present.")
    ...
    def request(self) -> Tuple[bytes, int, Mapping[str, str]]:
        raise NotImplementedError("AIOHTTPClient does not support synchronous requests.")
    ...
    def _get_request_args_kwargs(self, method: str, url: str, headers: Mapping[str, str], post_data):
        ...
        if self._proxy:
            if self._proxy["http"] != self._proxy["https"]:
                raise ValueError("AIOHTTPClient does not support different proxies for HTTP and HTTPS.")
    ...
    async def request_async(
        ...
        (
            ...
            status_code,
            response_headers,
        ) = await self.request_stream_async(method, url, headers, post_data=post_data)
