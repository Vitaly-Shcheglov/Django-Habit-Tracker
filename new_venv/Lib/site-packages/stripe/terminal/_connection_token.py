...
class ConnectionToken(CreateableAPIResource["ConnectionToken"]):
    ...
    OBJECT_NAME: ClassVar[Literal["terminal.connection_token"]] = "terminal.connection_token"
    ...
    @classmethod
    def create(cls, **params: Unpack["ConnectionToken.CreateParams"]) -> "ConnectionToken":
        """
    ...
    @classmethod
    async def create_async(cls, **params: Unpack["ConnectionToken.CreateParams"]) -> "ConnectionToken":
        """
        To connect to a reader the Stripe Terminal SDK needs to retrieve a short-lived connection token from Stripe, proxied through your server. On your backend, add an endpoint that creates and returns a connection token.
        """
        return cast(
