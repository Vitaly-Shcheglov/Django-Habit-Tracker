...
class SentinelConnectionPool(ConnectionPool):
    ...
    def __init__(self, service_name, sentinel_manager, **kwargs):
        kwargs["connection_class"] = kwargs.get(
            "connection_class",
            (SentinelManagedSSLConnection if kwargs.pop("ssl", False) else SentinelManagedConnection),
        )
    ...
    def __repr__(self):
        ...
        return f"<{type(self).__module__}.{type(self).__name__}" f"(service={self.service_name}({role}))>"
...
class Sentinel(SentinelCommands):
    ...
    def __repr__(self):
        sentinel_addresses = []
        for sentinel in self.sentinels:
            sentinel_addresses.append("{host}:{port}".format_map(sentinel.connection_pool.connection_kwargs))
        return f"<{type(self).__module__}.{type(self).__name__}" f"(sentinels=[{','.join(sentinel_addresses)}])>"
