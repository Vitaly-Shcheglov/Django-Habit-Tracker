...
try:
    ...
    HIREDIS_AVAILABLE = int(hiredis_version[0]) > 3 or (int(hiredis_version[0]) == 3 and int(hiredis_version[1]) >= 2)
...
def warn_deprecated_arg_usage(
    ...
    msg = f"Call to '{function_name}' function with deprecated" f" usage of input argument/s '{arg_name}'."
    if reason:
        msg += f" ({reason})"
    if version:
        msg += f" -- Deprecated since version {version}."
