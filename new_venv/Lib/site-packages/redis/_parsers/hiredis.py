...
from .base import (AsyncBaseParser, AsyncPushNotificationsParser, BaseParser,
                   PushNotificationsParser)
from .socket import (NONBLOCKING_EXCEPTION_ERROR_NUMBERS,
                     NONBLOCKING_EXCEPTIONS, SENTINEL,
    ...
    def read_response(self, disable_decoding=False, push_request=False):
        ...
        if self._next_response is not NOT_ENOUGH_DATA:
            ...
            if self._hiredis_PushNotificationType is not None and isinstance(
                ...
                if not push_request:
                    return self.read_response(disable_decoding=disable_decoding, push_request=push_request)
                else:
                    return response
            return response
...
class _AsyncHiredisParser(AsyncBaseParser, AsyncPushNotificationsParser):
    ...
    async def read_response(
        ...
        if isinstance(response, ConnectionError):
            ...
            if not push_request:
                return await self.read_response(disable_decoding=disable_decoding, push_request=push_request)
            else:
                return response
        elif isinstance(response, list) and response and isinstance(response[0], ConnectionError):
            raise response[0]
