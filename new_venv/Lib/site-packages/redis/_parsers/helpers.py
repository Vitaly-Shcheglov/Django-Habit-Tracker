...
def parse_acl_getuser(response, **options):
    ...
    if "selectors" in data:
        if data["selectors"] != [] and isinstance(data["selectors"][0], list):
            ...
            data["selectors"] = [
                {str_if_bytes(k): str_if_bytes(v) for k, v in selector.items()} for selector in data["selectors"]
            ]
...
_RedisCallbacksRESP2 = {
    ...
    **string_keys_to_dict(
        "ZDIFF ZINTER ZPOPMAX ZPOPMIN ZRANGE ZRANGEBYSCORE ZRANK ZREVRANGE " "ZREVRANGEBYSCORE ZREVRANK ZUNION",
        zset_score_pairs,
    ),
...
_RedisCallbacksRESP3 = {
    ...
    **string_keys_to_dict("XREAD XREADGROUP", parse_xread_resp3),
    ...
    "CONFIG GET": lambda r: {
        str_if_bytes(key) if key is not None else None: (str_if_bytes(value) if value is not None else None)
    ...
    "XINFO CONSUMERS": lambda r: [{str_if_bytes(key): value for key, value in x.items()} for x in r],
