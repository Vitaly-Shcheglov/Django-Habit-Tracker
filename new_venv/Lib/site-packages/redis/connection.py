...
from redis.cache import (CacheEntry, CacheEntryStatus, CacheFactory,
                         CacheFactoryInterface, CacheInterface, CacheKey)
...
from .exceptions import (AuthenticationError,
                         ...
                         ChildDeadlockedError, ConnectionError, DataError,
                         RedisError, ResponseError, TimeoutError)
from .retry import Retry
from .utils import (CRYPTOGRAPHY_AVAILABLE, HIREDIS_AVAILABLE, SSL_AVAILABLE,
                    compare_versions, deprecated_args, ensure_string,
                    format_error_message, get_lib_version, str_if_bytes)
...
class AbstractConnection(ConnectionInterface):
    ...
    def connect_check_health(self, check_health: bool = True):
        if self._sock:
            return
        try:
            sock = self.retry.call_with_retry(lambda: self._connect(), lambda error: self.disconnect(error))
        ...
        *,
        ...
        try:
            if self.protocol in ["3", 3]:
                response = self._parser.read_response(disable_decoding=disable_decoding, push_request=push_request)
            else:
...
class CacheProxyConnection(ConnectionInterface):
    ...
    def connect(self):
        ...
        if server_name != self.DEFAULT_SERVER_NAME or compare_versions(server_ver, self.MIN_ALLOWED_VERSION) == 1:
    ...
    def send_command(self, *args, **kwargs):
        ...
        # Creates cache key.
        self._current_command_cache_key = CacheKey(command=args[0], redis_keys=tuple(kwargs.get("keys")))
    ...
    def read_response(self, disable_decoding=False, *, disconnect_on_error=True, push_request=False):
        with self._cache_lock:
            ...
            if (
                ...
                and self._cache.get(self._current_command_cache_key).status != CacheEntryStatus.IN_PROGRESS
            ):
...
class SSLConnection(Connection):
    ...
    def _wrap_socket_with_ssl(self, sock):
        ...
        if self.ca_certs is not None or self.ca_path is not None or self.ca_data is not None:
            context.load_verify_locations(cafile=self.ca_certs, capath=self.ca_path, cadata=self.ca_data)
        ...
        if self.ssl_validate_ocsp_stapled and self.ssl_validate_ocsp:
            raise RedisError("Either an OCSP staple or pure OCSP connection must be validated " "- not both.")
...
class ConnectionPool:
        ...
        if connection_kwargs.get("cache_config") or connection_kwargs.get("cache"):
            ...
            if cache is not None:
                ...
                if self._cache_factory is not None:
                    ...
                    self.cache = CacheFactory(self.connection_kwargs.get("cache_config")).get_cache()
    ...
    def make_connection(self) -> "ConnectionInterface":
        ...
        if self.cache is not None:
            return CacheProxyConnection(self.connection_class(**self.connection_kwargs), self.cache, self._lock)
    ...
    def re_auth_callback(self, token: TokenInterface):
        with self._lock:
            for conn in self._available_connections:
                ...
                conn.retry.call_with_retry(lambda: conn.read_response(), lambda error: self._mock(error))
            for conn in self._in_use_connections:
...
class BlockingConnectionPool(ConnectionPool):
    ...
    def make_connection(self):
        ...
        if self.cache is not None:
            connection = CacheProxyConnection(self.connection_class(**self.connection_kwargs), self.cache, self._lock)
        else:
