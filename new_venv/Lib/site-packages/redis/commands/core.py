...
from typing import (TYPE_CHECKING, Any, AsyncIterator, Awaitable, Callable,
                    Dict, Iterable, Iterator, List, Literal, Mapping, Optional,
...
from redis.typing import (AbsExpiryT, AnyKeyT, BitfieldOffsetT, ChannelT,
                          ...
                          FieldT, GroupT, KeysT, KeyT, Number, PatternT,
                          ResponseT, ScriptTextT, StreamIdT, TimeoutSecT,
                          ZScoreBoundT)
        ...
        if passwords:
            ...
            for i, password in enumerate(passwords):
                ...
                if password.startswith(b"+"):
                    ...
                    raise DataError(f"Password {i} must be prefixed with a " f'"+" to add or a "-" to remove')
        ...
        if hashed_passwords:
            ...
            for i, hashed_password in enumerate(hashed_passwords):
                ...
                if hashed_password.startswith(b"+"):
                    ...
                    raise DataError(f"Hashed password {i} must be prefixed with a " f'"+" to add or a "-" to remove')
...
class ManagementCommands(CommandsProtocol):
    ...
    def client_list(self, _type: Optional[str] = None, client_id: List[EncodableT] = [], **kwargs) -> ResponseT:
        """
    ...
    def slaveof(self, host: Optional[str] = None, port: Optional[int] = None, **kwargs) -> ResponseT:
        """
...
class BasicKeyCommands(CommandsProtocol):
    ...
    def hrandfield(self, key: str, count: Optional[int] = None, withvalues: bool = False) -> ResponseT:
        """
    ...
    def set(
        ...
        if len(opset) > 2 or len(opset) > 1 and keepttl:
            raise DataError("``ex``, ``px``, ``exat``, ``pxat``, " "and ``keepttl`` are mutually exclusive.")
...
class ScanCommands(CommandsProtocol):
    ...
    def hscan_iter(
        ...
        cursor = "0"
        while cursor != 0:
            cursor, data = self.hscan(name, cursor=cursor, match=match, count=count, no_values=no_values)
            if no_values:
                yield from data
            else:
...
class AsyncScanCommands(ScanCommands):
    ...
    async def hscan_iter(
        ...
        cursor = "0"
        while cursor != 0:
            cursor, data = await self.hscan(name, cursor=cursor, match=match, count=count, no_values=no_values)
            if no_values:
                for it in data:
                    yield it
            else:
...
class SortedSetCommands(CommandsProtocol):
    ...
    def zinter(self, keys: KeysT, aggregate: Optional[str] = None, withscores: bool = False) -> ResponseT:
        """
    ...
    def zrandmember(self, key: KeyT, count: Optional[int] = None, withscores: bool = False) -> ResponseT:
        """
...
class HashCommands(CommandsProtocol):
    ...
    def hsetex(
        ...
        if items and len(items) % 2 != 0:
            raise DataError("'hsetex' with odd number of items. " "'items' must contain a list of key/value pairs.")
        ...
        return self.execute_command("HSETEX", name, *exp_options, "FIELDS", int(len(pieces) / 2), *pieces)
...
class ScriptCommands(CommandsProtocol):
    ...
    def _eval(self, command: str, script: str, numkeys: int, *keys_and_args: str) -> Union[Awaitable[str], str]:
...
class GeoCommands(CommandsProtocol):
    ...
    def geodist(self, name: KeyT, place1: FieldT, place2: FieldT, unit: Optional[str] = None) -> ResponseT:
        """
...
class FunctionCommands:
    ...
    def _fcall(self, command: str, function, numkeys: int, *keys_and_args: Any) -> Union[Awaitable[str], str]:
