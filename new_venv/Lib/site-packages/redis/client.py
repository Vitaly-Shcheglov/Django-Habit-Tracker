...
from redis.connection import (AbstractConnection, Connection, ConnectionPool,
                              SSLConnection, UnixDomainSocketConnection)
from redis.credentials import CredentialProvider
from redis.event import (AfterPooledConnectionsInstantiationEvent,
                         AfterPubSubConnectionInstantiationEvent,
                         AfterSingleConnectionInstantiationEvent, ClientType,
                         EventDispatcher)
from redis.exceptions import (ConnectionError, ExecAbortError, PubSubError,
                              RedisError, ResponseError, WatchError)
from redis.lock import Lock
from redis.retry import Retry
from redis.utils import (_set_info_logger, deprecated_args, get_lib_version,
                         safe_str, str_if_bytes, truncate_text)
    ...
    def __init__(
        ...
        if not connection_pool:
            ...
            self._event_dispatcher.dispatch(
                AfterPooledConnectionsInstantiationEvent([connection_pool], ClientType.SYNC, credential_provider)
            )
...
class PubSub:
    ...
    def on_connect(self, connection) -> None:
        ...
        if self.shard_channels:
            shard_channels = {self.encoder.decode(k, force=True): v for k, v in self.shard_channels.items()}
        ...
        while self.health_check_response_counter > 0 and ttl > 0:
            if self._execute(conn, conn.can_read, timeout=conn.socket_timeout):
                ...
                if self.is_health_check_response(response):
                    ...
                    raise PubSubError(
                        "A non health check response was cleaned by " "execute_command: {}".format(response)
                    )
            ttl -= 1
    ...
    def run_in_thread(
        ...
        thread = PubSubWorkerThread(self, sleep_time, daemon=daemon, exception_handler=exception_handler)
        thread.start()
        return thread
...
class PubSubWorkerThread(threading.Thread):
    def __init__(
        ...
        sleep_time: float,
        daemon: bool = False,
        exception_handler: Union[Callable[[Exception, "PubSub", "PubSubWorkerThread"], None], None] = None,
    ):
        super().__init__()
        self.daemon = daemon
        self.pubsub = pubsub
