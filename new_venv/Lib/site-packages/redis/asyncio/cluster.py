...
from redis.cluster import (PIPELINE_BLOCKED_COMMANDS, PRIMARY, REPLICA,
                           ...
                           LoadBalancingStrategy, block_pipeline_command,
...
from redis.exceptions import (AskError, BusyLoadingError, ClusterDownError,
                              ClusterError, ConnectionError,
                              CrossSlotTransactionError, DataError,
                              ExecAbortError, InvalidPipelineStack,
                              MaxConnectionsError, MovedError,
...
from redis.utils import (SSL_AVAILABLE, deprecated_args, deprecated_function,
                         get_lib_version, safe_str, str_if_bytes,
                         truncate_text)
            ...
            try:
                ...
                if len(target_nodes) == 1:
                    ...
                    values = await asyncio.gather(
                        *(asyncio.create_task(self._execute_command(node, *args, **kwargs)) for node in target_nodes)
                    )
                    if command in self.result_callbacks:
...
class ClusterNode:
    ...
    def __repr__(self) -> str:
        return f"[host={self.host}, port={self.port}, " f"name={self.name}, server_type={self.server_type}]"
    ...
    async def re_auth_callback(self, token: TokenInterface):
        ...
        while self._free:
            ...
            await conn.retry.call_with_retry(lambda: conn.read_response(), lambda error: self._mock(error))
...
class NodesManager:
            ...
            for slot in cluster_slots:
                ...
                for replica_node in replica_nodes:
                    ...
                    if not target_replica_node:
                        target_replica_node = ClusterNode(host, port, REPLICA, **self.connection_kwargs)
                    # add this node to the nodes cache
    ...
    async def aclose(self, attr: str = "nodes_cache") -> None:
        ...
        await asyncio.gather(*(asyncio.create_task(node.disconnect()) for node in getattr(self, attr).values()))
    ...
    async def _execute(
        ...
        for cmd in todo:
            ...
            if passed_targets and not client._is_node_flag(passed_targets):
                ...
                if not target_nodes:
                    raise RedisClusterException(f"No targets were found to execute {cmd.args} command on")
            if len(target_nodes) > 1:
...
class TransactionStrategy(AbstractStrategy):
    ...
    async def _get_connection_and_send_command(self, *args, **options):
        ...
        return await self._send_command_parse_response(connection, redis_node, args[0], *args, **options)
