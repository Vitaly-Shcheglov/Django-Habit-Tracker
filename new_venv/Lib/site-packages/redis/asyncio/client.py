...
from typing import (TYPE_CHECKING, Any, AsyncIterator, Awaitable, Callable,
                    Dict, Iterable, List, Mapping, MutableMapping, Optional,
                    Protocol, Set, Tuple, Type, TypedDict, TypeVar, Union,
                    cast)
...
from redis.utils import (SSL_AVAILABLE, _set_info_logger, deprecated_args,
                         deprecated_function, get_lib_version, safe_str,
                         str_if_bytes, truncate_text)
...
class Redis(AbstractRedis, AsyncRedisModuleCommands, AsyncCoreCommands, AsyncSentinelCommands):
        ...
        *,
        ...
        if not connection_pool:
            ...
            self._event_dispatcher.dispatch(
                AfterPooledConnectionsInstantiationEvent([connection_pool], ClientType.ASYNC, credential_provider)
            )
        else:
    ...
    def __repr__(self):
        return f"<{self.__class__.__module__}.{self.__class__.__name__}" f"({self.connection_pool!r})>"
...
class Pipeline(Redis):  # lgtm [py/init-calls-subclass]
    ...
    def annotate_exception(self, exception: Exception, number: int, command: Iterable[object]) -> None:
        ...
        msg = f"Command # {number} ({truncate_text(cmd)}) " "of pipeline caused error: {exception.args}"
