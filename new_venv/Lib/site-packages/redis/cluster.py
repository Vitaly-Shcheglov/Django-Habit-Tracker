...
from redis.cache import (CacheConfig, CacheFactory, CacheFactoryInterface,
                         CacheInterface)
...
from redis.event import (AfterPooledConnectionsInstantiationEvent,
                         AfterPubSubConnectionInstantiationEvent, ClientType,
                         EventDispatcher)
from redis.exceptions import (AskError, AuthenticationError, ClusterDownError,
                              ClusterError, ConnectionError,
                              CrossSlotTransactionError, DataError,
                              ExecAbortError, InvalidPipelineStack, MovedError,
                              RedisClusterException, RedisError, ResponseError,
                              SlotNotCoveredError, TimeoutError, TryAgainError,
                              WatchError)
from redis.lock import Lock
from redis.retry import Retry
from redis.utils import (deprecated_args, dict_merge, list_keys_to_dict,
                         merge_result, safe_str, str_if_bytes, truncate_text)
...
def cleanup_kwargs(**kwargs):
    ...
    """
    connection_kwargs = {k: v for k, v in kwargs.items() if k in REDIS_ALLOWED_KEYS and k not in KWARGS_DISABLED_KEYS}
...
class AbstractRedisCluster:
    ...
    RESULT_CALLBACKS = dict_merge(
        ...
        list_keys_to_dict(["CLIENT UNBLOCK"], lambda command, res: 1 if sum(res.values()) > 0 else 0),
        list_keys_to_dict(["SCAN"], parse_scan_result),
        list_keys_to_dict(["SCRIPT LOAD"], lambda command, res: list(res.values()).pop()),
        list_keys_to_dict(["SCRIPT EXISTS"], lambda command, res: [all(k) for k in zip(*res.values())]),
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if "retry" in kwargs:
            ...
            raise RedisClusterException("The 'retry' argument cannot be used in kwargs when running in cluster mode.")
...
class NodesManager:
    ...
    def get_node_from_slot(
        ...
        if self.slots_cache.get(slot) is None or len(self.slots_cache[slot]) == 0:
            raise SlotNotCoveredError(
                f'Slot "{slot}" not covered by the cluster. ' f'"require_full_coverage={self._require_full_coverage}"'
            )
    ...
    def create_redis_connections(self, nodes):
        ...
        self._event_dispatcher.dispatch(
            AfterPooledConnectionsInstantiationEvent(connection_pools, ClientType.SYNC, self._credential_provider)
        )
    ...
    def create_redis_node(self, host, port, **kwargs):
        ...
        node_retry_config = Retry(backoff=NoBackoff(), retries=0, supported_errors=(ConnectionError,))
    ...
    def initialize(self):
        ...
        if not startup_nodes_reachable:
            raise RedisClusterException(
                f"Redis Cluster cannot be connected. Please provide at least " f"one reachable node: {str(exception)}"
            ) from exception
...
def block_pipeline_command(name: str) -> Callable[..., Any]:
    ...
    def inner(*args, **kwargs):
        raise RedisClusterException(
            f"ERROR: Calling pipelined function {name} is blocked " f"when running redis in cluster mode..."
        )
...
class AbstractStrategy(ExecutionStrategy):
    ...
    def annotate_exception(self, exception, number, command):
        ...
        msg = f"Command # {number} ({truncate_text(cmd)}) of pipeline " f"caused error: {exception.args[0]}"
...
class PipelineStrategy(AbstractStrategy):
    ...
    def unlink(self, *names):
        if len(names) != 1:
            raise RedisClusterException("unlinking multiple keys is not implemented in pipeline command")
...
class TransactionStrategy(AbstractStrategy):
    ...
    def __init__(self, pipe: ClusterPipeline):
        ...
        self._retry.update_supported_errors(RedisCluster.ERRORS_ALLOW_RETRY + self.SLOT_REDIRECT_ERRORS)
    ...
    def _get_client_and_connection_for_transaction(self) -> Tuple[Redis, Connection]:
        ...
        if not self._pipeline_slots:
            raise RedisClusterException("At least a command with a key is needed to identify a node")

        node: ClusterNode = self._nodes_manager.get_node_from_slot(list(self._pipeline_slots)[0], False)
    ...
    def execute_command(self, *args, **kwargs):
        ...
        if (self._watching or args[0] in self.IMMEDIATE_EXECUTE_COMMANDS) and not self._explicit_transaction:
            if args[0] == "WATCH":
                self._validate_watch()

            if slot_number is not None:
                if self._pipeline_slots and slot_number not in self._pipeline_slots:
                    raise CrossSlotTransactionError("Cannot watch or send commands on different slots")

                self._pipeline_slots.add(slot_number)
            elif args[0] not in self.NO_SLOTS_COMMANDS:
                raise RedisClusterException(
                    f"Cannot identify slot number for command: {args[0]}," "it cannot be triggered in a transaction"
                )
    ...
    def _execute_transaction(self, stack: List["PipelineCommand"], raise_on_error: bool):
        if len(self._pipeline_slots) > 1:
            raise CrossSlotTransactionError("All keys involved in a cluster transaction must map to the same slot")
