    ...
    def decorator(view_method):
        ...
        if methods or method:
            ...
            assert not isinstance(methods, str), (
                "`methods` expects to receive a list of methods;" " use `method` for a single argument"
            )
            if method:
        ...
        if available_http_methods:
            ...
            assert not any(
                hasattr(getattr(view_cls, mth, None), "_swagger_auto_schema") for mth in _methods
    ...
    if default is not serializers.empty:
        ...
        if default is not serializers.empty and default is not None:
            try:
                ...
                logger.warning(
                    "'default' on schema for %s will not be set because " "to_representation raised an exception",
                    field,
                    exc_info=True,
                )
                default = serializers.empty
