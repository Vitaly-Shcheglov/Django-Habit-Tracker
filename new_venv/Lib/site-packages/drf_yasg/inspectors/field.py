    ...
    def get_request_parameters(self, serializer, in_):
        ...
        parameters = [
            self.probe_field_inspectors(
                value, openapi.Parameter, self.use_definitions, name=self.get_parameter_name(key), in_=in_
            )
        ...
        if isinstance(field, (serializers.ListSerializer, serializers.ListField)):
            ...
            def make_schema_definition(serializer=field):
                ...
                for property_name, child in serializer.fields.items():
                    ...
                    child_schema = self.probe_field_inspectors(child, ChildSwaggerType, use_references, **prop_kwargs)
...
if version.parse(drf_version) < version.parse("3.14.0"):
    model_field_to_basic_type.append((models.NullBooleanField, (openapi.TYPE_BOOLEAN, None)))
...
def get_basic_type_info(field):
    ...
    result = OrderedDict([("type", swagger_type), ("format", format), ("pattern", pattern)])
...
class ChoiceFieldInspector(FieldInspector):
    ...
    def field_to_swagger_object(self, field, swagger_object_type, use_references, **kwargs):
        ...
        if isinstance(field, serializers.ChoiceField):
            ...
            if isinstance(field, serializers.MultipleChoiceField):
                result = SwaggerType(type=openapi.TYPE_ARRAY, items=ChildSwaggerType(type=enum_type, enum=enum_values))
...
class DictFieldInspector(FieldInspector):
    ...
    def field_to_swagger_object(self, field, swagger_object_type, use_references, **kwargs):
        ...
        if isinstance(field, serializers.DictField) and swagger_object_type == openapi.Schema:
            child_schema = self.probe_field_inspectors(field.child, ChildSwaggerType, use_references)
            return SwaggerType(type=openapi.TYPE_OBJECT, additional_properties=child_schema)
...
class CamelCaseJSONFilter(FieldInspector):
    ...
    if CamelCaseJSONParser and CamelCaseJSONRenderer:
        ...
        def is_camel_case(self):
            return any(issubclass(parser, CamelCaseJSONParser) for parser in self.get_parser_classes()) or any(
                issubclass(renderer, CamelCaseJSONRenderer) for renderer in self.get_renderer_classes()
            )
