...
class SwaggerAutoSchema(ViewInspector):
    ...
    def add_manual_parameters(self, parameters):
        ...
        if any(param.in_ == openapi.IN_FORM for param in manual_parameters):  # pragma: no cover
            ...
            if self.method not in self.body_methods:
                raise SwaggerGenerationError(
                    "form parameters can only be applied to " "(" + ",".join(self.body_methods) + ") HTTP methods"
                )
    ...
    def get_responses(self):
        ...
        return openapi.Responses(responses=self.get_response_schemas(response_serializers))
    ...
    def get_query_parameters(self):
        ...
        if query_serializer is not None:
            ...
            if (
                len(set(param_list_to_odict(natural_parameters)) & set(param_list_to_odict(serializer_parameters)))
                != 0
            ):
                raise SwaggerGenerationError(
