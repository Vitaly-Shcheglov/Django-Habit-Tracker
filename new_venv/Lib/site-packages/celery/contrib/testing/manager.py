...
class ManagerMixin:
            ...
            try:
                ...
                self.remark(
                    "Still waiting for {}/{}: [{}]: {!r}".format(
                        len(r) - len(received), len(r), truncate(", ".join(waiting_for)), exc
                    ),
                    "!",
                )
    ...
    def assert_received(self, ids, interval=0.5, desc="waiting for tasks to be received", **policy):
        return self.assert_task_worker_state(self.is_received, ids, interval=interval, desc=desc, **policy)

    def assert_result_tasks_in_progress_or_completed(
        self, async_results, interval=0.5, desc="waiting for tasks to be started or completed", **policy
    ):
        return self.assert_task_state_from_result(
            self.is_result_task_in_progress, async_results, interval=interval, desc=desc, **policy
        )
    ...
    def assert_task_worker_state(self, fun, ids, interval=0.5, **policy):
        return self.wait_for(partial(self.true_or_raise, fun, ids, timeout=interval), (Sentinel,), **policy)
