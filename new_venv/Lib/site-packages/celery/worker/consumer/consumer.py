...
from celery.exceptions import (CPendingDeprecationWarning, InvalidTaskError,
                               NotRegistered, WorkerShutdown, WorkerTerminate)
...
from celery.worker.state import (active_requests, maybe_shutdown, requests,
...
debug, info, warn, error, crit = (logger.debug, logger.info, logger.warning, logger.error, logger.critical)
...
class Consumer:
    ...
    def _get_connection_retry_type(self, is_connection_loss_on_startup):
        return (
            ...
            if (is_connection_loss_on_startup and self.app.conf.broker_connection_retry_on_startup is not None)
    ...
    def connection_for_write(self, url=None, heartbeat=None):
        return self.ensure_connected(self.app.connection_for_write(url=url, heartbeat=heartbeat))
