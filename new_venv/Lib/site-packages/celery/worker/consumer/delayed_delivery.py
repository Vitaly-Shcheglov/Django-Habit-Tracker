...
class DelayedDelivery(bootsteps.StartStopStep):
    ...
    def start(self, c: Consumer) -> None:
        ...
        for broker_url in broker_urls:
            try:
                ...
                logger.warning("Failed to setup delayed delivery for %r: %s", broker_url, str(e))
    ...
    def _setup_delayed_delivery(self, c: Consumer, broker_url: str) -> None:
        ...
        logger.debug("Setting up delayed delivery for broker %r with queue type %r", broker_url, queue_type)

        try:
            declare_native_delayed_delivery_exchanges_and_queues(connection, queue_type)
        except Exception as e:
            logger.warning("Failed to declare exchanges and queues for %r: %s", broker_url, str(e))
            raise
    ...
    def _validate_queue_type(self, queue_type: Optional[str]) -> None:
        ...
        if queue_type not in VALID_QUEUE_TYPES:
            ...
            raise ValueError(f"Invalid queue type {queue_type!r}. Must be one of: {', '.join(sorted_types)}")
