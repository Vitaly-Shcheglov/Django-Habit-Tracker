...
class SecureSerializer:
    ...
    def serialize(self, data):
        ...
        with reraise_errors("Unable to serialize: {0!r}", (Exception,)):
            content_type, content_encoding, body = dumps(data, serializer=self._serializer)
    ...
    def deserialize(self, data):
        ...
        return loads(body, payload["content_type"], payload["content_encoding"], force=True)

    def _pack(self, body, content_type, content_encoding, signer, signature, sep=DEFAULT_SEPARATOR):
        fields = sep.join(
            ensure_bytes(s) for s in [b64encode(signer), b64encode(signature), content_type, content_encoding, body]
        )
...
def register_auth(
    key=None, key_password=None, cert=None, store=None, digest=DEFAULT_SECURITY_DIGEST, serializer="json"
):
