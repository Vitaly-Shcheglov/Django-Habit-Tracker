...
class Autoscale(ParamType):
    ...
    def convert(self, value, param, ctx):
        ...
        if len(value) > 2:
            self.fail("Expected two comma separated integers or one integer." f"Got {len(value)} instead.")
        ...
        try:
            ...
            self.fail("Expected two comma separated integers." f"Got {value.join(',')} instead.")
...
def detach(
    ...
    with detached(logfile, pidfile, uid, gid, umask, workdir, fake, after_forkers=False):
        try:
...
@click.option(
    ...
    help_group="Worker Options",
    help="Path to the state database. The extension '.db' may be " "appended to the filename.",
)
@click.option(
    "-l",
    "--loglevel",
...
@click.option("--purge", "--discard", is_flag=True, cls=CeleryOption, help_group="Queue Options")
@click.option("--queues", "-Q", type=COMMA_SEPARATED_LIST, cls=CeleryOption, help_group="Queue Options")
@click.option("--exclude-queues", "-X", type=COMMA_SEPARATED_LIST, cls=CeleryOption, help_group="Queue Options")
@click.option("--include", "-I", type=COMMA_SEPARATED_LIST, cls=CeleryOption, help_group="Queue Options")
