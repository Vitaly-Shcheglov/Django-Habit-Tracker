...
class DynamoDBBackend(KeyValueStoreBackend):
    ...
    def _get_table_schema(self):
        ...
        return {
            "AttributeDefinitions": [
                {"AttributeName": self._key_field.name, "AttributeType": self._key_field.data_type}
            ],
    ...
    def _get_ttl_specification(self, ttl_attr_name):
        ...
        return {
            "TableName": self.table_name,
            "TimeToLiveSpecification": {"Enabled": self._has_ttl(), "AttributeName": ttl_attr_name},
        }
    ...
    def _get_table_ttl_description(self):
        ...
        try:
            ...
            logger.error(
                ("Error describing Time to Live on DynamoDB table {table}: " "{code}: {message}").format(
    ...
    def _set_table_ttl(self):
        ...
        if status in ("ENABLED", "ENABLING"):
            ...
            if not self._has_ttl():
                ...
                logger.debug(
                    ("DynamoDB Time to Live is {situation} " "on table {table}").format(
                        situation="already disabled" if status == "DISABLED" else "currently being disabled",
                        table=self.table_name,
                    )
                )
                return description
        ...
        # a log showing the unknown state.
            ...
            logger.warning(
                ("Unknown DynamoDB Time to Live status {status} " "on table {table}. Attempting to continue.").format(
                    status=status, table=self.table_name
                )
            )
        ...
        try:
            ...
            logger.info(
                ("DynamoDB table Time to Live updated: " "table={table} enabled={enabled} attribute={attr}").format(
                    table=self.table_name, enabled=self._has_ttl(), attr=self._ttl_field.name
                )
            )
            ...
            error_message = e.response["Error"].get("Message", "Unknown")
            logger.error(
                ("Error {action} Time to Live on DynamoDB table {table}: " "{code}: {message}").format(
    ...
    def _prepare_put_request(self, key, value):
        ...
        put_request = {
            ...
            "Item": {
                ...
                self._timestamp_field.name: {self._timestamp_field.data_type: str(timestamp)},
            },
        }
        if self._has_ttl():
            put_request["Item"].update(
                {self._ttl_field.name: {self._ttl_field.data_type: str(int(timestamp + self.time_to_live_seconds))}}
            )
        return put_request
