...
try:
    ...
    pydocumentdb = DocumentClient = ConsistencyLevel = PartitionKind = HTTPFailure = ConnectionPolicy = (
        RetryOptions
    ) = None
...
class CosmosDBSQLBackend(KeyValueStoreBackend):
        ...
        **kwargs,
        ...
        try:
            self._consistency_level = getattr(
                ConsistencyLevel, consistency_level or conf["cosmosdbsql_consistency_level"]
            )
    ...
    def _client(self):
        ...
        connection_policy.RetryOptions = RetryOptions(
            max_retry_attempt_count=self._max_retry_attempts, max_wait_time_in_seconds=self._max_retry_wait_time
        )
    ...
    def _create_collection_if_not_exists(self, client):
        try:
            client.CreateCollection(
                self._database_link,
                {"id": self._collection_name, "partitionKey": {"paths": ["/id"], "kind": PartitionKind.Hash}},
            )
        except HTTPFailure as ex:
            if ex.status_code != ERROR_EXISTS:
                raise
