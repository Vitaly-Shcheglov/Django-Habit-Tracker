...
class Analysis:
    ...
    def narrow(self, lines: Container[TLineNo]) -> Analysis:
        ...
        if self.has_arcs:
            arc_possibilities_set = {(a, b) for a, b in self.arc_possibilities_set if a in lines or b in lines}
            arcs_executed_set = {(a, b) for a, b in self.arcs_executed_set if a in lines or b in lines}
    ...
    def arcs_missing(self) -> list[TArc]:
        ...
        missing = (
            ...
            if p not in self.arcs_executed_set and p[0] not in self.no_branch and p[1] not in self.excluded
        )
