...
class PyTracer(Tracer):
    ...
    def log(self, marker: str, *args: Any) -> None:
        ...
        with open("/tmp/debug_trace.txt", "a", encoding="utf-8") as f:
            ...
            if 0:  # if you want callers..
                ...
                stack = " / ".join((fname or "???").rpartition("/")[-1] for _, fname, _, _ in self.data_stack)
                f.write(stack)
            f.write("\n")
    ...
    def _trace(
        ...
        if (
            ...
            sys.settrace(None)
            try:
                self.cur_file_data, self.cur_file_name, self.last_line, self.started_context = self.data_stack.pop()
            except IndexError:
                self.log(
                    "Empty stack!",
