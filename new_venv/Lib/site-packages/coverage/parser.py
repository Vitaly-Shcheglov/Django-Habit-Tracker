...
class PythonParser:
            ...
            if isinstance(node, (ast.ClassDef, ast.FunctionDef, ast.AsyncFunctionDef, ast.Module)):
                if node.body:
                    ...
                    if (
                        ...
                        self.raw_docstrings.update(range(first.lineno, cast(int, first.end_lineno) + 1))
    ...
    def parse_source(self) -> None:
        ...
        try:
            ...
            raise NotPython(
                f"Couldn't parse '{self.filename}' as Python source: " + f"{err.args[0]!r} at line {lineno}",
            ) from err
...
class ByteParser:
    ...
    def child_parsers(self) -> Iterable[ByteParser]:
        ...
        """
        return (ByteParser(self.text, code=c) for c in code_objects(self.code) if c.co_name != "__annotate__")
...
class AstArcAnalyzer:
    ...
    def with_jump_fixers(self) -> dict[TArc, tuple[TArc, TArc]]:
        ...
        """
        fixers = {}
        with_nexts = {arc for arc in self.arcs if arc[0] in self.all_with_starts and arc not in self.with_entries}
