        ...
        for lineno, tokens in enumerate(fr.source_token_lines(), start=1):
            ...
            if lineno in analysis.excluded:
                ...
                if len(mba) == branch_stats[lineno][0]:
                    ...
                    long_annotations.append(f"line {lineno} didn't jump anywhere: it always raised an exception.")
                else:
...
class HtmlReporter:
    ...
    def report(self, morfs: Iterable[TMorf] | None) -> float:
        ...
        return self.index_pages["file"].totals.n_statements and self.index_pages["file"].totals.pc_covered
    ...
    def write_html_page(self, ftr: FileToReport) -> None:
        ...
        for ldata in file_data.lines:
            ...
            if ldata.long_annotations:
                ...
                assert len(longs) == 1, f"Had long annotations in {ftr.fr.relative_filename()}: {longs}"
                ...
                for region in regions:
                    ...
                    page_data.summaries.append(
                        IndexItem(
                            ...
                            file=escape(ftr.fr.relative_filename()),
                            description=(f"<data value='{escape(sorting_name)}'>" + escape(region.name) + "</data>"),
                            nums=analysis.numbers,
                        )
                    )
                ...
                if self.should_report(analysis, page_data):
                    page_data.summaries.append(
                        IndexItem(
                            ...
                            file=escape(ftr.fr.relative_filename()),
                            description=(
                                "<data value=''>" + f"<span class='no-noun'>(no {escape(noun)})</span>" + "</data>"
                            ),
                            nums=analysis.numbers,
                        )
                    )
