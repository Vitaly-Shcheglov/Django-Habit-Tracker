    ...
    def map(self, path: str, exists: Callable[[str], bool] = source_exists) -> str:
        ...
        for original_pattern, regex, result in self.aliases:
            if m := regex.match(path):
                ...
                if not exists(new):
                    self.debugfn(
                        f"Rule {original_pattern!r} changed {path!r} to {new!r} " + "which doesn't exist, continuing",
                    )
                    continue
                self.debugfn(
                    f"Matched path {path!r} to rule {original_pattern!r} -> {result!r}, " + f"producing {new!r}",
                )
                return new
