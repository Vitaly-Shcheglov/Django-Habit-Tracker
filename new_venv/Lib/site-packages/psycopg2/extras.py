...
class LoggingConnection(_connection):
    ...
    def _check(self):
        if not hasattr(self, "_logobj"):
            raise self.ProgrammingError("LoggingConnection object has not been initialize()d")
        ...
        if slot_type == REPLICATION_LOGICAL:
            ...
            if output_plugin is not None:
                raise psycopg2.ProgrammingError(
                    "cannot specify output plugin name when creating " "physical replication slot"
                )

            command += "PHYSICAL"

        else:
            raise psycopg2.ProgrammingError(f"unrecognized replication type: {repr(slot_type)}")
...
def register_inet(oid=None, conn_or_curs=None):
    ...
    warnings.warn("the inet adapter is deprecated, it's not very useful", DeprecationWarning)
    ...
    _ext.INET = _ext.new_type((oid1,), "INET", lambda data, cursor: data and Inet(data) or None)
    ...
    def parse(self, s, cur, _bsdec=_re.compile(r"\\(.)")):
        ...
        for m in self._re_hstore.finditer(s):
            if m is None or m.start() != start:
                raise psycopg2.InterfaceError(f"error parsing hstore pair at char {start}")
        ...
        if start < len(s):
            raise psycopg2.InterfaceError(f"error parsing hstore: unparsed data after char {start}")
...
class CompositeCaster:
    ...
    def __init__(self, name, oid, attrs, array_oid=None, schema=None):
        ...
        if array_oid:
            self.array_typecaster = _ext.new_array_type((array_oid,), f"{name}ARRAY", self.typecaster)
        else:
...
def _split_sql(sql):
    ...
    for token in tokens:
        ...
        if token[1:] == b"s":
            ...
            raise ValueError("unsupported format character: '%s'" % token[1:].decode("ascii", "replace"))
