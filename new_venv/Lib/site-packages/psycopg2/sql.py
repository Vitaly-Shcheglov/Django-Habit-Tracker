    ...
    def __init__(self, seq):
        ...
        for i in seq:
            if not isinstance(i, Composable):
                raise TypeError(f"Composed elements must be Composable, got {i!r} instead")
            wrapped.append(i)
...
class SQL(Composable):
    ...
    def format(self, *args, **kwargs):
        ...
        for pre, name, spec, conv in _formatter.parse(self._wrapped):
            ...
            if name.isdigit():
                if autonum:
                    raise ValueError("cannot switch from automatic field numbering to manual")
                rv.append(args[int(name)])
                autonum = None

            elif not name:
                if autonum is None:
                    raise ValueError("cannot switch from manual field numbering to automatic")
